...
export class PokemonService {
  ...
  private getPokemonGeneric(limit: number, offset: number): Promise<IPokemonRawResponse> {
    const pokemonUrl = `${process.env.POKEMON_BASE_URL}/pokemon?offset=${offset}&limit=${limit}`;

    return fetchData(pokemonUrl);
  }

  private getPokemonSpecies(id: string): Promise<IPokemonSpeciesRawResponse> {
